...
// 生成 package.json
function generatePackageJson(framework: typeof frameworks[0]) {
  ...
  return {
    ...
    dependencies: {
      ...
      ...Object.fromEntries(framework.deps.map(dep => [dep, 'latest'])),
    },
    devDependencies: {
      typescript: '^5.3.0',
      vite: '^5.0.0',
      ...Object.fromEntries(framework.devDeps.map(dep => [dep, 'latest'])),
    },
  }
}

// 生成 vite.config.ts
function generateViteConfig(framework: typeof frameworks[0]) {
...
// 生成 index.html
function generateIndexHtml(framework: typeof frameworks[0]) {
  return `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
// 生成 README.md
function generateReadme(framework: typeof frameworks[0]) {
...
// 创建目录结构
function createExampleStructure(framework: typeof frameworks[0]) {
  const packagePath = join(process.cwd(), 'packages', framework.name)
...
console.log('\n✨ 所有示例项目创建完成!')
console.log('\n📝 提示: 需要手动创建每个框架的源代码文件 (App 组件和 main 入口)')
