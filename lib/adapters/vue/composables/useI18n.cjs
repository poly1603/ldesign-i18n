...
function useI18n(options = {}) {
  ...
  const globalI18n = vue.inject(constants.I18N_SYMBOL);
  if (!globalI18n) {
  ...
  const locale = vue.ref(i18n.locale || "en_us");
  const fallbackLocale = vue.ref(i18n.fallbackLocale || "en_us");
  const stopWatchLocale = vue.watchEffect(() => {
    i18n.locale = locale.value;
  });
  const stopWatchFallback = vue.watchEffect(() => {
  ...
  const messages = vue.computed(() => i18n.getMessages(locale.value) || {});
