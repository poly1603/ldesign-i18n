...
class LRUCache {
  ...
  calculateEvictionScore(entry) {
    ...
    return (frequency * 1e3 + priority * 1e4) / (recency + age);
  }
  calculateSize(value) {
...
class SmartCache extends node_events.EventEmitter {
  constructor() {
    super();
  ...
  async set(key, value, options) {
    ...
    if (hotCache) {
      hotCache.set(key, value, {
        ...
        tags: options?.tags ? new Set(options.tags) : void 0
      });
    }
    this.emit("cache:set", {
      key,
  ...
  async invalidateByTag(tag) {
    const invalidated = 0;
    this.emit("cache:invalidate:tag", {
      tag,
      count: invalidated
    });
    return invalidated;
  ...
  recordAccess(key) {
    ...
    if (this.accessPatterns.size > this.maxAccessPatterns) {
      const keysToDelete = Array.from(this.accessPatterns.keys()).slice(0, Math.floor(this.maxAccessPatterns / 2));
  ...
  getStats() {
    const layerStats = /* @__PURE__ */ new Map();
    let totalHits = 0;
    let totalMisses = 0;
  ...
  handleMemoryPressure(level) {
    ...
    this.emit("cache:memory:pressure", {
      level
    });
  }
  destroy() {
