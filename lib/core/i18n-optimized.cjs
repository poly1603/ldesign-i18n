...
class OptimizedI18n {
  constructor(config = {}) {
    ...
    Object.defineProperty(this, "eventEmitter", {
      ...
      configurable: true,
      writable: true,
      value: new helpers.EventEmitter()
    });
    ...
    this.interpolation = new interpolation.InterpolationEngine(config.interpolation);
    this.pluralization = new pluralization.PluralizationEngine(config.pluralSeparator);
    this.cache = config.cache === false ? new cache.LRUCache(0) : cache.createCache(typeof config.cache === "object" ? config.cache : {
      maxSize: 1e3
    });
  ...
  translateBatch(keys, options = {}) {
    ...
    for (let i = 0; i < keyList.length; i++) {
      ...
      if (result === void 0) {
        let message = messages ? helpers.getNestedValue(messages, key, this.keySeparator) : void 0;
        if (message === void 0) {
  ...
  resolveMessageOptimized(key, locale, namespace, options) {
    ...
    if (nsIndex > -1) {
      ...
      if (nsMessages) {
        const value2 = helpers.getNestedValue(nsMessages, resolvedKey, this.keySeparator);
        if (value2 !== void 0 && helpers.isString(value2)) {
          return value2;
        }
      }
    }
    const value = helpers.getNestedValue(messages, resolvedKey, this.keySeparator);
    if (value !== void 0) {
      if (helpers.isString(value)) {
        return value;
      } else if (helpers.isPlainObject(value) && options.count !== void 0) {
  ...
  mergeMessages(locale, messages, namespace) {
    ...
    const merged = helpers.deepMerge({}, existing, messages);
  ...
  handleMissing(key, locale, namespace, options) {
    if (this.isDev) {
      ...
      if (this.config?.warnOnMissing !== false) {
        helpers.warn(`Missing translation for key "${key}" in locale "${locale}"`);
      }
    }
  ...
  detectLocale() {
    ...
    return helpers.getBrowserLanguage();
  }
...
exports.FastCacheKeyBuilder = FastCacheKeyBuilder;
exports.I18n = OptimizedI18n;
exports.ObjectPool = ObjectPool;
exports.OptimizedI18n = OptimizedI18n;
/*! End of @ldesign/i18n | Powered by @ldesign/builder */
//# sourceMappingURL=i18n-optimized.cjs.map
